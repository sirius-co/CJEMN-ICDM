...
def classification(X, y, test_ids, val_ids, train_ids):
    ...
    clf = LogisticRegressionCV(max_iter=5000, cv=10, class_weight='balanced', solver='sag')
    ...
    print(f"Micro-F1: {best_test_mi_f1}, Macro-F1: {best_test_ma_f1}")
    
    return best_test_mi_f1, best_test_ma_f1



def run_kmeans(X, y, k, test_ids, dim_):
...
def run_kmeans_test_centroid_dim(X, y, k, centroids, test_ids):
...
def cluster_centroids(nodeEmb, attEmb, gt, clusters, test_ids, dim):
    ...
    nmi1, ari1, ami1, dim1 = run_kmeans(nodeEmb, gt, clusters, test_ids, dim)
    # Run clustering with the pre-computed centroids.
    nmi2, ari2, ami2, dim2 = run_kmeans_test_centroid_dim(nodeEmb, gt, clusters, centroids, test_ids)

    
    print("Without Centroids:")
    print(f"NMI = {nmi1}, ARI = {ari1}, AMI = {ami1}\n")
    print("With Centroids Clustering:")
    print(f"NMI = {nmi2}, ARI = {ari2}, AMI = {ami2}\n")
    
    return nmi1, ari1, ami1, nmi2, ari2, ami2
