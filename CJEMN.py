...
class Embeddings:
    ...
    def run(self):
        ...
        while iteration < self.iterr or converged == False:
            ...
            if actualCost < minCost - threshold:
                ...
                best_nodeEmbedding = self.nodeEmbedding.clone()
                best_attributeEmbedding = self.attributeLabelEmbedding.clone()
            elif minCostID + self.extraiter >= iteration:
                converged = False
            else:
...
class ReadGraph:
    ...
    s = 1                 # number of neighbors for kNN
    ...
    def readACM(self):
        ...
        # Add self-loops to the adjacency matrices       
        ...
        train_ids = np.sort(loadtxt(base_path + 'old/train_ids_1.txt')).astype(int)
        test_ids  = np.sort(loadtxt(base_path + 'old/test_ids_1.txt')).astype(int)
        val_ids   = np.sort(loadtxt(base_path + 'old/val_ids_1.txt')).astype(int)
    ...
    def readIMDB(self):
        ...
        self_loop_mask = torch.eye(self.nodes, dtype=torch.float32, device=device)
        adj1 = adj1 + self_loop_mask
        adj2 = adj2 + self_loop_mask
    ...
    def readFREEBASE(self):
        ...
        adj1 = torch.tensor(data['mam'].todense(), dtype=torch.float32, device=device)
        adj2 = torch.tensor(data['mdm'].todense(), dtype=torch.float32, device=device)
        adj3 = torch.tensor(data['mwm'].todense(), dtype=torch.float32, device=device)
        self.nodes = adj1.size(0)

        self_loop_mask = torch.eye(self.nodes, dtype=torch.float32, device=device)
        adj1 = adj1 + self_loop_mask
        adj2 = adj2 + self_loop_mask
        adj3 = adj3 + self_loop_mask
    ...
    def readFLICKR(self):
        ...
        adj1 = torch.tensor(scipy.io.loadmat(base_path + "layer0mat.mat")['layer0mat'], dtype=torch.float32, device=device)
        adj2 = torch.tensor(scipy.io.loadmat(base_path + "layer1mat.mat")['layer1mat'], dtype=torch.float32, device=device)
        self.nodes = adj1.size(0)
...
class MixedSpectral:
    def __init__(self, num_g, adj, num_nodes, weighted, attributes, numAtt, clusters):
        ...
        self.adj = [a.clone().detach().to(device).float() for a in adj[:self.num_g]]
...
embeddings = Embeddings(graphs, adj, num_nodes, node_attr, num_atts, dim, iterations,
                            ...
                            sumWeights, clusters, train_idx, test_idx, s, gt)
...
mi_ac, ma_ac = classification(nodeEmb.cpu().numpy(), gt, test_ids, val_ids, train_ids)  

print("\nNode Clustering Task: ")
nmi1, ari1, ami1, nmi2, ari2, ami2 = cluster_centroids(nodeEmb.cpu().numpy(), attEmb.cpu().numpy(), gt, clusters,  test_ids, dim_cluster)

print()

fh = open("results_" + dataset.name.upper() + "_CJEMN.txt", "a")
fh.write('Mi-F1 = %f, Ma-F1 = %f, NMI = %f, ARI = %f, AMI = %f, NMI = %f, ARI = %f, AMI = %f' % (mi_ac, ma_ac, nmi1, ari1, ami1, nmi2, ari2, ami2))
fh.write('\r\n')
fh.write('---------------------------------------------------------------------------------------------------')
fh.write('\r\n')
fh.flush()
